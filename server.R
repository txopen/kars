...
function(input, output, session) {
  ...
  datasetCands <- reactive({
    ...
    validate(
      if(ukf() == 1){
        need(histoc:::uk_candidate_dataframe_check(data) ,
             "Candidates' file is not valid for UK algorithm!")
    } else {
      need(histoc:::candidate_dataframe_check(data),
           "Candidates' file is not valid!")}
      )
  ...
  datasetDonors <- reactive({
    ...
    validate(
      if(ukf() == 1){
        need(histoc:::uk_donors_dataframe_check(data), 
             "Donors' file is not valid for UK algorithm!")
      } else {
      need(histoc:::donors_dataframe_check(data), 
           "Donors' file is not valid!")}
    )
